    ...
    <form class="col-xs-10 col-md-6 item row" align="left"  style="margin-left: 0px" class="navbar-form margin-none navbar-left row">
        ...
        <div class="col-xs-2 col-md-1 " style="margin-top: 8px " >
            <a  href="javascript:void(0);" type="submit" value="pretraga" class="btn btn-white" size="15px" height="10px"
                style="display: inline-block" onclick="search_moderator()">
...
<script >
    function search_moderator()
    {
        ...
        if(sel=='clanovi') {
            $.ajax({
                type: 'GET',
                url: '<?php echo site_url('moderator/get_pretraga_clan')?>/' + selectText,
                success: function (returnData) {
            ...
            $.ajax({
                type: 'GET',
                url: '<?php echo site_url('moderator/get_pretraga_kurs')?>/' + selectText,
                success: function (returnData) {
                    $('#nesto').html(returnData);
                }
            });
            ...
            $.ajax({
                type: 'GET',
                url: '<?php echo site_url('moderator/get_pretraga_predavac')?>/' + selectText,
                success: function (returnData) {
                    $('#nesto').html(returnData);
                }
            });
